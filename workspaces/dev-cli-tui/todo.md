...
### 7. 单元测试与自测
- [x] 编写 CLI 参数解析的单元测试（已添加 parse_size 与 Clap 解析测试，3 项通过）
- [x] 新增 JSON 输出结构单元测试（serde_json 验证顶层与关键字段）
- [ ] 编写进度条和输出格式的集成测试（保留）
- [ ] 创建自测脚本，验证基本功能
- [ ] 在工作区记录自测结果
- [ ] 创建自测脚本，验证基本功能
...
## 备注
- 当前 surf-core 版本已提供同步扫描与基础统计：`top_files`、`by_extension`、`stale_files` 已初步实现并通过单元测试；后续迭代将优化性能与进度回调。
