...
impl TaskManager {
    ...
    fn collect_results_if_needed(&self, task_id: &str) -> Result<CollectedResults, JsonRpcError> {
        ...
        if let Some(ref cached) = info.aggregated_result {
            return Ok(CollectedResults {
                path: info.path.clone(),
                limit: info.limit,
                aggregated_result: cached.clone(),
            });
        }
        ...
        let entries = match scan_handle {
            Some(handle) => match surf_core::collect_results(handle) {
                Ok(entries) => entries,
                Err(e) => {
            ...
            None => {
                ...
                // 返回内部错误
                let detail = format!(
                    "internal error: scan handle missing for completed task {}",
                    task_id
                );
                return Err(JsonRpcError::invalid_params(Some(detail)));
            }
                                ...
                                Ok(collected) => {
                                    ...
                                    let entries_vec: Vec<AggregatedEntry> = if mode == "summary" {
                                        ...
                                        match effective_limit {
                                            ...
                                            _ => collected.aggregated_result.entries.clone(),
                                        }
                                    };
...
async fn main() -> anyhow::Result<()> {
    ...
    println!(
        "surf-service listening on {addr} (max_concurrent_scans={max}, task_ttl_seconds={ttl}).\nJSON-RPC service ready: Surf.Scan / Surf.Status / Surf.Cancel / Surf.GetResults 已与 surf-core 进度感知扫描 API 与结果聚合打通（含任务登记、进度查询、取消与结果收集语义；并发上限与任务 TTL 仍作为后续增强项保留）。",
        addr = addr,
