...
## 问题 1
- **报告 Agent**: design-architect
- **相关目录**: Architecture.md
- **问题描述**: 删除操作在不同平台（Linux/macOS）和不同形态（CLI/TUI/GUI）下的统一策略需要确认。具体问题：
  1. Linux 平台下是否存在统一「回收站」语义，还是一律视为永久删除？
  2. 各形态（CLI/TUI/GUI）是否应遵循同一删除语义和确认流程？
- **人类决策**: （请在此行下方填写决策）
- 设计建议: 
  1. 删除语义统一为「软删除优先」：在支持平台上优先移动到回收站/废纸篓，仅在用户显式选择或环境不支持时执行永久删除。 同意
  2. Linux：系统本身没有像 macOS 那样的统一废纸篓 API，但常见桌面环境遵循 XDG Trash 规范。核心库应内置对 XDG Trash 的最佳努力支持（将文件移动到 `~/.local/share/Trash` 等），不强制依赖 `trash-cli` 等外部工具；如果检测到环境不满足 Trash 条件，则退化为永久删除，并要求上层 UI 以「将被永久删除」字样进行提示。默认策略：Linux TUI 默认采用永久删除 + 二次确认，用户可通过配置开启「尽可能移动到 Trash」。 同意
  3. macOS：GUI 和 TUI 删除均通过核心库的 `MoveToTrash` 模式实现，与 Finder 的废纸篓行为保持一致；暂不提供默认永久删除入口，若未来增加「永久删除」，必须单独选项且附带更强的二次确认文案。 同意
  4. 形态与确认流程：当前版本 CLI 不暴露删除入口，仅 TUI 和 macOS GUI 支持删除；两者在执行删除前均需要二次确认，并在确认文案中明确说明是「移至废纸篓」还是「永久删除」。 TUI不需要支持删除功能, TUI仅做扫描即可
  5. 接口层：核心扫描引擎提供统一 `delete_entry(path, mode)` 能力，JSON-RPC 服务层提供 `file.delete` 方法并接受 `mode: "trash" | "permanent"`；具体字段与默认值见 `Architecture.md` 第 4.1、5.3 节。
