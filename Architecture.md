...
- **各工作区本地构建约定（Rust 部分）**
  ...
  - `dev-service-api`（工作区根：`workspaces/dev-service-api/`）
    ...
    - 现实状态注记（iteration 80 / delivery）：本轮在仓库根目录直接运行 `bash test/scripts/service_jsonrpc_basic.sh`，脚本在首次 `Surf.Status` 调用时收到空响应并以 `EXIT_CODE:1` 标记 FAIL；检查 `release/linux-x86_64/service/surf-service` 的启动输出，可见与上轮相同的占位提示（`JSON-RPC methods (Surf.Scan / Surf.Status / Surf.GetResults / Surf.Cancel) are not implemented yet; this binary ...`），确认当前 release 目录下的服务二进制仍为旧版占位实现，尚未包含 `workspaces/dev-service-api/surf-service/src/main.rs` 中已实现的 `Surf.Scan` / `Surf.Status` / `Surf.GetResults` / `Surf.Cancel` 逻辑。下一步需要在具备正常 Rust 依赖环境（可访问 crates.io 或预热完整依赖缓存）的开发机或 CI 上修复 `clap`/`clap_builder` 依赖问题、重新构建 `surf-service` 并替换 `release/linux-x86_64/service/surf-service`，然后再以 `test/scripts/service_jsonrpc_basic.sh` 和 `test/scripts/service_jsonrpc_invalid_params.sh` 作为 JSON-RPC 基本功能与错误路径的交付验收基线。
