...
- acceptance_criteria:
 ...
 - remaining_todos:
   ...
   - 方向三：CLI 表格模式下错误路径与默认参数的端到端回归——当前 CLI 集成测试已经在 JSON 模式下覆盖了非法 `--min-size` / `--threads` 和不存在路径的错误行为，并在非 JSON 模式下验证了非法 `--min-size` 场景；但尚未在“表格模式 + 不存在路径”以及“未显式传入 `--limit` 时默认展示前 20 条”的路径上编写独立用例。后续建议在 `workspaces/dev-cli-tui/surf-cli/tests/integration.rs` 补充上述场景，明确断言：在表格模式下遇到不存在路径时 stdout 不输出表头/数据行、stderr 含有“Failed to scan ... does not exist”等提示且退出码非零；在默认 `--limit` 时，扫描一个包含超过 20 个符合条件文件的目录并验证数据行数不超过 20，从而将本 story 的所有验收条目纳入自动化测试覆盖范围。
