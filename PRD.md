...
- acceptance_criteria:
  ...
  - impl_notes (iteration 17 / dev-cli-tui):
   ...
   - 通过上述参数检查后，`main` 会调用 `tui::run_tui(&args)` 进入真实的全屏 TUI 事件循环（`workspaces/dev-cli-tui/surf-cli/src/main.rs:173-187`），不再简单打印“尚未实现”占位提示。`tui::run_tui` 在进入备用屏幕并启用原始模式后，构造 `ScanConfig { root, min_size, threads }` 并调用 `surf-core::start_scan` 启动后台扫描；随后在事件循环中周期性调用 `poll_status(&handle)` 获取 `scanned_files` / `scanned_bytes` / `done` / `error` 等字段，并用 `ratatui` 渲染包含标题、内容区域与状态栏的三段式布局（`workspaces/dev-cli-tui/surf-cli/src/tui.rs:45-140`）。
   - 事件循环内部显式建模了 `TuiMode`（`Scanning` / `Browsing` / `Error`）和 `TuiExit`：在 `Scanning` 模式下，内容区域显示当前扫描根路径、已扫描文件数与字节数，并在文案中提示“Future features: Browse and navigate / Safe delete”；当 `StatusSnapshot.done == true` 且 `error.is_none()` 时，TUI 会通过 `surf_core::collect_results(handle)` 收集完整 `Vec<FileEntry>` 结果、切换到 `Browsing` 模式，并在内容区域用一个简单的列表视图展示前若干条按大小排序的结果（每行包含 size 和 path，当前选中条目前缀为 `▶`，参见 `workspaces/dev-cli-tui/surf-cli/src/tui.rs:82-139`）。在 `Browsing` 模式下，用户可以使用 `↑/↓` 或 `k/j` 移动当前选中条目，状态栏会展示“当前条目序号 / 列表总数”，但尚未支持目录下钻或删除操作；当 `StatusSnapshot.done == true` 且存在错误时，TUI 进入 `Error` 模式，内容区域展示错误摘要并提示用户按 `q`/`Esc` 退出。
   - 键盘事件处理方面，TUI 在每一帧之后以非阻塞方式轮询键盘事件：当用户按下 `q` 或 `Esc` 时，若扫描尚未完成则在 `Scanning` 模式下通过 `surf_core::cancel(&handle)` 发出“最佳努力”取消信号，然后退出事件循环；若已经进入 `Browsing` 或 `Error` 模式（扫描已结束），则直接退出（正常退出路径统一视为 `TuiExit::Completed`）。当用户在任一模式下按下 Ctrl+C（`KeyCode::Char('c')` 且 `key.modifiers` 包含 `KeyModifiers::CONTROL`）时，如果扫描仍在进行则调用 `surf_core::cancel(&handle)` 请求取消，并将 `exit_reason` 标记为 `TuiExit::Interrupted` 后跳出循环；主程序在接收到 `TuiExit::Interrupted` 时，会在 stderr 打印 "Scan interrupted by user (Ctrl+C) in TUI mode" 并以 130 非零退出码结束进程，从而与 CLI 单次模式的 Ctrl+C 语义对齐：扫描过程中按 Ctrl+C 会尝试取消核心扫描、不输出部分结果且以 130 退出码结束进程（`workspaces/dev-cli-tui/surf-cli/src/main.rs:159-189`）。
   - 基于上述实现，本 story 已在“Scanning 状态下的进度反馈”和“扫描完成后基于结果列表的基础浏览能力（支持上下移动选中条目）”以及“在 TUI 内发起取消请求并退出”的行为上完成了第一步落地：TUI 模式下的 Ctrl+C 行为已与 CLI 单次模式保持一致（均不输出部分结果，退出码为 130），扫描完成后用户可以在同一界面查看按大小排序的 TopN 列表并用方向键或 `j/k` 在其中移动选中行。但仍未提供目录树/目录聚合视图、目录下钻/返回导航，以及“移入回收站”的删除功能，因此 story 状态仍保持 `status: todo`；后续迭代需要围绕 `Browsing` / `ConfirmDelete` / `Deleting` 等状态机以及目录聚合与删除适配层补全剩余验收条目，并在 TUI 中引入详情窗格与删除确认弹窗以完全满足本 story 的验收标准。
