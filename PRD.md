...
- acceptance_criteria:
  ...
  - impl_notes (iteration 14 / dev-cli-tui):
   ...
   - 事件处理方面，TUI 在每一帧之后以非阻塞方式轮询键盘事件：当用户按下 `q` 或 `Esc` 时，若扫描尚未完成则调用 `surf_core::cancel(&handle)` 发出“最佳努力”取消信号，然后退出事件循环；若扫描已完成或出错，则直接退出（正常退出路径统一视为 `TuiExit::Completed`）。当用户在 TUI 中按下 Ctrl+C（`KeyCode::Char('c')` 且 `key.modifiers` 包含 `KeyModifiers::CONTROL`）时，若扫描尚未完成同样调用 `surf_core::cancel(&handle)`，并将 `exit_reason` 标记为 `TuiExit::Interrupted` 后跳出循环（`workspaces/dev-cli-tui/surf-cli/src/tui.rs:70-181`）；主程序在接收到 `TuiExit::Interrupted` 时，会在 stderr 打印 "Scan interrupted by user (Ctrl+C) in TUI mode" 并以 130 非零退出码结束进程，从而与 CLI 单次模式的 Ctrl+C 语义对齐：扫描过程中按 Ctrl+C 会尝试取消核心扫描、不输出部分结果且以 130 退出码结束进程（`workspaces/dev-cli-tui/surf-cli/src/main.rs:159-189`）。
   - 基于上述实现，本 story 在“Scanning 状态下的进度反馈”和“在 TUI 内发起取消请求并退出”的行为上已经有了最小落地：TUI 模式下的 Ctrl+C 行为已在当前迭代中落地，并与 CLI 单次模式保持一致（均不输出部分结果，退出码为 130），但尚未提供目录树/列表浏览、键盘导航与“移入回收站”的删除功能；因此 story 状态仍保持 `status: todo`，后续迭代需要围绕 `Browsing` / `ConfirmDelete` / `Deleting` 等状态机以及目录聚合与删除适配层补全剩余验收条目。
