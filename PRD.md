...
- acceptance_criteria:
  ...
  - impl_notes (iteration 20 / dev-cli-tui):
   ...
   - 基于上述实现，本 story 已在“Scanning 状态下的进度反馈”“扫描完成后基于结果列表的基础浏览能力（支持上下移动选中条目及右侧详情窗格）”以及“在 TUI 内发起取消请求并退出”和“对单个文件执行带二次确认的安全删除（移入回收站）”等行为上完成了重要子集：TUI 模式下的 Ctrl+C 行为已与 CLI 单次模式保持一致（均不输出部分结果，退出码为 130），扫描完成后用户可以在同一界面查看按大小排序的 TopN 列表、浏览当前选中条目的详情，并通过 `d`→`y`/Enter 将单个文件移入系统回收站。当前 TUI 已基于扫描结果构建多级目录树视图：用户可以在根目录与子目录之间通过 Enter / Backspace / `h` / `l` 等按键进行下钻和返回，列表按聚合后的 size 降序展示；删除能力目前仍仅支持单个文件，目录级删除以及删除后对祖先目录聚合大小的精确更新尚未实现，同时 release 目录下的 CLI 二进制及交付层面的 TUI 冒烟脚本（例如未来的 `test/scripts/tui_basic_navigation.sh`）尚未与上述实现对齐，因此 story 状态暂维持 `status: todo`，后续迭代需补齐目录删除与聚合更新，并在交付工件与测试脚本中完成端到端验收，以完全满足本 story 的验收标准。
